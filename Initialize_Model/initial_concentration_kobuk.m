...
Floe3 = initialize_floe_values(B3,height);
Floe4 = initialize_floe_values(B4,height);
...
for jj = 1:Ny
    for ii = 1:Nx
        if c(jj,ii)>0
            ...
            polyB = intersect(c2_boundary_poly,boundary); %Use these when having
%             boundaries
            N = 4*ceil(NumFloes*area(polyB)/area(c2_boundary_poly)/c(jj,ii)); %Use these when having
