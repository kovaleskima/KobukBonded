...
for ii =1:length(Floe)
    ...
    for jj = 1:length(Floe(ii).bonds)
        if Floe(ii).num<Floe(ii).bonds(jj).Num
            Xb1 = Floe(ii).bonds(jj).Xb; Yb1 = Floe(ii).bonds(jj).Yb; P_bond(:,count) = A_rot*[Xb1;Yb1]+[Floe(ii).Xi;Floe(ii).Yi];
            count = count+1;
        end
    end
end
