...
ratio=Ly/Lx;%max(ocean.Yo)/max(ocean.Xo);
...
for ii = 1:length(Floes)
    ...
    Xb = [Xb; cat(1,Floes(ii).bonds.Xb)+Xi(ii)]; 
    Xc = [Xc; cat(1,Floes(ii).bonds.Xc)+Xi(ii)]; 
    Yb = [Yb; cat(1,Floes(ii).bonds.Yb)+Yi(ii)]; 
    Yc = [Yc; cat(1,Floes(ii).bonds.Yc)+Yi(ii)]; 
