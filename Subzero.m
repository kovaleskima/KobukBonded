...
% nPar = 3; %Number of workers for parfor
...
%     Nbond = length(Areas(Areas<bond_area+1));
    ...
    if AVERAGE
        ...
        U = U+squeeze(eularian_data.u);V = V+squeeze(eularian_data.v);
        %U = U +cat(1,Floe.Ui); V = V+cat(1,Floe.Ui);
        dU = dU+squeeze(eularian_data.du);dV = dV+squeeze(eularian_data.dv); 
        %Fx = Fx+squeeze(eularian_data.force_x);Fy = Fy+squeeze(eularian_data.force_y);
%         ext_force = cat(1,Floe.collision_force);
%         Fx = Fx + ext_force(:,1); Fy = Fy + ext_force(:,2);
        Sig = Sig+squeeze(eularian_data.stress);
...
%     if max(c2_boundary(2,:)) > 49000 && mod(i_step,20)==0 && side == 1
%         xb = c2_boundary(1,:);
%         yb = c2_boundary(2,:);
%         xb = xb + 2.5*[-1 -1 1 1];
...
%             side = 2;
%         end
%     elseif max(c2_boundary(1,:)) > 49000 && mod(i_step,20)==0 && side == 2
%         xb = c2_boundary(1,:);
%         yb = c2_boundary(2,:);
%         xb = xb - 2.5*[-1 -1 1 1];
%         yb = yb + 2.5*[-1 1 1 -1];%     y=[-1 1 1 (-1-0.1*Time*f/pi*sin(f*Time)) -1]*Ly;
